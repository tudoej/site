 ...
 *
 * @package   ThemeFramework\BreadcrumbTrail
 * @version   0.6.1
...
class Breadcrumb_Trail {
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => __( 'Browse:',                 APP_TD ),
			'home'                => __( 'Home',                    APP_TD ),
			'search'              => __( 'Search results for "%s"', APP_TD ),
			'error_404'           => __( '404 Not Found',           APP_TD ),
			'paged'               => __( 'Page %d',                 APP_TD ),
			'archives'            => __( 'Archives',                APP_TD ),
			'archive_minute_hour' => __( 'g:i a',                   APP_TD ),
			'archive_minute'      => __( 'Minute %d',               APP_TD ),
			'archive_hour'        => __( 'g a',                     APP_TD ),
			'archive_day'         => __( 'd',                       APP_TD ),
			'archive_week'        => __( 'Week %d',                 APP_TD ),
			'archive_month'       => __( 'F',                       APP_TD ),
			'archive_year'        => __( 'Y',                       APP_TD ),
		//	'edit'                => __( 'Edit',                    APP_TD ), // @todo Implement edit link
		);
	...
	public function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '" title="' . get_the_time( __( 'Y', APP_TD ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_year'], $post_id ) . '</a>';

				/* If using the %monthnum% tag, add a link to the monthly archive. */
				elseif ( '%monthnum%' == $tag )
					$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F Y', APP_TD ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_month'], $post_id ) . '</a>';

				/* If using the %day% tag, add a link to the daily archive. */
				elseif ( '%day%' == $tag )
					$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F j, Y', APP_TD ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_day'], $post_id ) . '</a>';
