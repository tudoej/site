<div id="sidebar">
	...
	<aside id="dashboard-acct-info">
		...
		<ul class="links">
			...
			<?php foreach ( va_get_available_user_networks( $dashboard_user->ID ) as $social_network => $account ) { ?>
			<li class="<?php echo esc_attr( $social_network ); ?>">
				<a href="<?php echo va_get_social_account_url( $social_network, $account ); ?>" title="<?php echo esc_attr( va_get_social_network_title( $social_network ) ); ?>" target="_blank"><?php echo $account ?></a>
			</li>
			<?php } ?>
	...
	<?php if ( ! $is_own_dashboard && $dashboard_user->description ) { ?>
		<aside id="dashboard-acct-about">
			<div class="section-head">
				<h3><?php printf( __( 'About %s', APP_TD ), $dashboard_user->display_name ); ?></h3>
			</div>
			<div class="user-description">
				<?php echo apply_filters( 'the_content', $dashboard_user->description ); ?>
			</div>
		</aside>
	<?php } ?>
